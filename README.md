...
The **Video FX** tab is the pool: one tick per video effect (the twenty of the Video tab, *pixel* included) and one per shader file, all on at first, for whoever cannot stand one of them (no more super8 look, ever). What is unticked is out of the show: an effect's strength counts as 0 whatever its knob says, the knob greys out in the Video tab and Randomize leaves it alone; a shader is left out of *random*, `Space`, the digit keys and the shader knob (which then run over the shorter pool, in list order) and only comes up when picked by name in the Video tab's combo; if the one on screen is unticked while the combo says random, another is drawn. *all* / *none* under each group; the ticks are remembered. Every tick is also a MIDI button, group *video fx: the pool* in the Controls tab, next to the knob rows that already drive every effect's strength and the shader choice.
